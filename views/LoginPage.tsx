  ...
  return (
    <div className="flex min-h-screen w-full bg-background-light dark:bg-background-dark">
      <div className="grid w-full grid-cols-1 md:grid-cols-2">
        {/* Left Side - Visual */}
        <div className="hidden md:flex flex-col items-center justify-center bg-cover bg-center relative p-12 text-center" 
            style={{backgroundImage: 'linear-gradient(rgba(16, 22, 34, 0.7) 0%, rgba(16, 22, 34, 0.9) 100%), url("https://picsum.photos/1200/1600")'}}>
            <div className="relative z-10 flex flex-col items-center">
                ...
                <h2 className="text-white text-4xl font-black leading-tight tracking-[-0.033em] mb-4">MODIVIS</h2>
                <p className="text-white/80 text-lg max-w-md">{t('landing.hero_desc')}</p>
            </div>
        ...
        <div className="flex flex-col justify-center items-center p-8 sm:p-12 md:p-16 bg-white dark:bg-background-dark">
            <div className="w-full max-w-sm space-y-8">
                <div className="text-center md:text-left">
                    ...
                    <h2 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{t('auth.login_title')}</h2>
                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{t('auth.login_subtitle')}</p>
                </div>
                ...
                <form className="space-y-6" onSubmit={handleLogin}>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center">
                            <input id="remember-me" name="remember-me" type="checkbox" className="h-4 w-4 rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-dark-surface text-primary focus:ring-primary" />
                            <label htmlFor="remember-me" className="ml-2 block text-sm text-slate-700 dark:text-slate-300">{t('auth.remember_me')}</label>
                        </div>

                        <div className="text-sm">
                            <a href="#" className="font-medium text-primary hover:text-primary/80">{t('auth.forgot_password')}</a>
                        </div>
                    </div>
                ...
                <p className="text-center text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <button onClick={() => onNavigate(AppView.SIGNUP)} className="font-bold text-primary hover:text-primary/80 transition-colors">
                        {t('auth.signup_title')}
                    </button>
                </p>
            </div>
